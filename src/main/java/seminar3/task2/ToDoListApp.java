...
public class ToDoListApp {
    ...
    public static void addNewTask(Scanner scanner, List<ToDoVers2> tasks) {
        System.out.println("Введите название новой задачи: ");
        String newTaskTitle = scanner.nextLine();
        tasks.add(new ToDoVers2(newTaskTitle));
    ...
    public static void displayTasks(List<ToDoVers2> tasks) {
        System.out.println("Список задач: ");
        for(int i = 0; i < tasks.size(); i++) {
            ToDoVers2 task = tasks.get(i);
            String status = task.isDone() ? "[x]" : "[ ]";
            System.out.println((i + 1) + ". " + status + " " + task.getTitle());
        }
    }

    public static void markTaskAsDone(Scanner scanner, List<ToDoVers2> tasks) {
    ...
    public static void saveTasksToFile(String fileName, List<ToDoVers2> tasks) {
        try {
    ...
    public static List<ToDoVers2> loadTasksFromFile(String fileName) {
        List<ToDoVers2> tasks = new ArrayList<>();

        File file = new File(fileName);
        if (file.exists()) {
            try {
                if (fileName.endsWith(".json")) {
                    tasks = objectMapper.readValue(file, objectMapper.getTypeFactory().constructCollectionType(List.class, ToDoVers2.class));
                } else if (fileName.endsWith(".bin")) {
                    try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                        tasks = (List<ToDoVers2>) ois.readObject();
                    }
                } else if (fileName.endsWith(".xml")) {
                    tasks = xmlMapper.readValue(file, xmlMapper.getTypeFactory().constructCollectionType(List.class, ToDoVers2.class));
                }
